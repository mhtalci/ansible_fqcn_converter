...
class TempProjectBuilder:
    ...
    def add_group_vars(self, group: str, vars_dict: Dict[str, Any]) -> 'TempProjectBuilder':
    ...
    def add_host_vars(self, host: str, vars_dict: Dict[str, Any]) -> 'TempProjectBuilder':
...
class AssertionHelpers:
    ...
    def assert_contains_fqcn(content: str, module_name: str, fqcn: str) -> None:
        ...
        for line in lines:
            if f"{module_name}:" in line and not line.strip().startswith('#'):
                ...
                stripped = line.strip()
                if stripped.startswith(f"- {module_name}:") or stripped.startswith(f"{module_name}:"):
                    raise AssertionError(f"Short module name '{module_name}:' still present in: {line}")
    ...
    def assert_preserves_parameters(content: str, parameter_name: str) -> None:
        ...
        for line in parameter_lines:
            ...
            if not line.startswith('  ') and not line.startswith('\t'):
                ...
                if stripped.startswith(f"- {parameter_name}:") or stripped.startswith(f"{parameter_name}:"):
                    continue  # This is legitimate module usage
            
            # Check that it's not converted to FQCN
            assert f"ansible.builtin.{parameter_name}:" not in line, \
    ...
    def assert_conversion_count(content: str, expected_count: int) -> None:
        ...
        assert fqcn_count >= expected_count, \
        ...
        for module in short_modules:
            ...
            for line_num, line in enumerate(lines, 1):
                if f"{module}:" in line and not line.strip().startswith('#'):
                    stripped = line.strip()
                    if stripped.startswith(f"- {module}:") or stripped.startswith(f"{module}:"):
...
class PerformanceHelpers:
    ...
    @staticmethod
    def generate_large_playbook(num_tasks: int, modules: Optional[List[str]] = None) -> str:
        """Generate a large playbook with specified number of tasks."""
        if modules is None:
            modules = ['package', 'service', 'copy', 'file', 'user', 'group', 'command', 'template']
...
"""
    ...
    @staticmethod
    def generate_complex_role_structure(num_roles: int, tasks_per_role: int) -> Dict[str, Dict[str, str]]:
...
""",
    ...
    def assert_validation_result(result, expected: Dict[str, Any]) -> None:
        ...
        if 'expected_valid' in expected:
            assert result.valid == expected['expected_valid'], \
                f"Expected valid={expected['expected_valid']}, got {result.valid}"
        
        if 'expected_score' in expected:
            assert abs(result.score - expected['expected_score']) < 0.01, \
