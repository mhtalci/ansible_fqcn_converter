...
"""
        ...
        (project1 / "inventory.ini").write_text(
            """[webservers]
web1.example.com
"""
        )
...
"""
    ...
    def test_batch_conversion_parallel(self, multiple_projects, temp_dir):
        ...
        start_time = time.time()
        sequential_results = batch_processor_sequential.process_projects(
            valid_projects, dry_run=True
        )
    ...
    def test_batch_conversion_actual(self, multiple_projects, temp_dir):
        ...
        project2_tasks = (
            multiple_projects["project2"] / "roles" / "database" / "tasks" / "main.yml"
        )
...
"""
        ...
        custom_projects = batch_processor.discover_projects(
            str(temp_dir), patterns=["deploy.yml", "provision.yaml"]
        )
...
"""
        ...
        (large_project / "site.yml").write_text(
            f"""---
...
"""
        ...
        for role in roles:
            (large_project / f"{role}s.yml").write_text(
                f"""---
...
"""
        ...
        assert (
            results[0]["files_processed"] >= len(roles) * 2 + len(roles) + 1
        )  # tasks + handlers + playbooks

        # Should have converted many modules
        assert (
            results[0]["modules_converted"] >= len(roles) * 4
        )  # At least 4 modules per role
