...
class QualityMetricsCollector:
    ...
    def _analyze_complexity(self, source_dir: Path) -> dict:
        ...
        complexity_data["overall_complexity"] = (
    ...
    def _analyze_code_style(self, source_dir: Path) -> dict:
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, "-m", "flake8", str(source_dir), "--format=json"],
                capture_output=True,
                text=True,
            )
    ...
    def _analyze_documentation(self, source_dir: Path) -> dict:
        ...
        for py_file in source_dir.rglob("*.py"):
            ...
            try:
                ...
                doc_metrics["undocumented_functions"].extend(
                    file_doc_data["undocumented_functions"]
                )
                doc_metrics["undocumented_classes"].extend(
                    file_doc_data["undocumented_classes"]
                )
        ...
        doc_metrics["overall_doc_coverage"] = (
            ...
            if isinstance(node, ast.FunctionDef) and node.name.startswith("test_"):
                ...
                for child in ast.walk(node):
                    if isinstance(child, ast.Call):
                        if isinstance(
                            child.func, ast.Name
                        ) and child.func.id.startswith("assert"):
    ...
    def _analyze_security(self, source_dir: Path) -> dict:
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, "-m", "bandit", "-r", str(source_dir), "-f", "json"],
                capture_output=True,
                text=True,
            )
            ...
            if result.stdout:
                ...
                security_metrics["total_issues"] = len(
                    security_metrics["security_issues"]
                )
...
class TestCoverageAndQualityReporting:
    ...
    def test_generate_comprehensive_coverage_report(
        self, coverage_analyzer, temp_report_dir
    ):
...
""",
        ...
        if overall_coverage < MINIMUM_COVERAGE:
            pytest.fail(
                f"Coverage {overall_coverage:.2f}% is below minimum threshold {MINIMUM_COVERAGE}%"
            )

        if overall_coverage < TARGET_COVERAGE:
            print(
                f"Warning: Coverage {overall_coverage:.2f}% is below target {TARGET_COVERAGE}%"
            )
        ...
        for filename, file_data in coverage_data["file_coverage"].items():
            if "core" in filename and file_data["coverage_percent"] < 85.0:
                print(
                    f"Warning: Core file {filename} has low coverage: {file_data['coverage_percent']:.2f}%"
                )
...
<coverage line-rate="0.85" branch-rate="0.80" version="1.9" timestamp="1640995200">
    ...
    <packages>
        ...
        required_files = [junit_xml, cobertura_xml, dashboard_json]
...
class TestQualityGates:
    ...
    def test_coverage_quality_gate(self, temp_dir):
        ...
        coverage_data = {
            ...
            "file_coverage": {
                ...
                "src/fqcn_converter/config/manager.py": {
        ...
        assert (
            coverage_data["overall_coverage"] >= MINIMUM_OVERALL_COVERAGE
        ...
        for filename, file_data in coverage_data["file_coverage"].items():
            if file_data["coverage_percent"] < MINIMUM_FILE_COVERAGE:
                low_coverage_files.append(
                    {"file": filename, "coverage": file_data["coverage_percent"]}
                )
    ...
    def test_complexity_quality_gate(self):
        ...
        assert (
            complexity_data["overall_complexity"] <= MAX_OVERALL_COMPLEXITY
    ...
    def test_security_quality_gate(self):
        ...
        assert (
            high_issues <= MAX_HIGH_SEVERITY_ISSUES
        ), f"High severity security issues: {high_issues} (max: {MAX_HIGH_SEVERITY_ISSUES})"

        assert (
            medium_issues <= MAX_MEDIUM_SEVERITY_ISSUES
        ...
        if security_data["total_issues"] > 0:
            ...
            for issue in security_data["issues"]:
                print(
                    f"  {issue['severity']}: {issue['test_name']} in {issue['filename']}:{issue['line_number']}"
                )
    ...
    def test_documentation_quality_gate(self):
        ...
        doc_data = {
            ...
            "undocumented_functions": [
                {
                    "file": "src/fqcn_converter/utils/helpers.py",
                    "function": "helper_function",
                },
                {
                    "file": "src/fqcn_converter/core/converter.py",
        ...
        assert (
            doc_data["overall_doc_coverage"] >= MINIMUM_DOC_COVERAGE
        ...
        public_undocumented = [
            func
            for func in doc_data["undocumented_functions"]
            if not func["function"].startswith("_")
        ]

        assert (
            len(public_undocumented) <= MAX_UNDOCUMENTED_PUBLIC_FUNCTIONS
