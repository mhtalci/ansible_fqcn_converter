...
"""
    ...
    def test_complete_project_conversion(self, real_ansible_project):
        ...
        webserver_tasks = (
            real_ansible_project / "roles" / "webserver" / "tasks" / "main.yml"
        )
    ...
    def test_conversion_with_validation(self, real_ansible_project):
        ...
        yaml_files = [
            f for f in real_ansible_project.rglob("*.yml") if self._is_ansible_file(f)
        ]
        ...
        average_score = sum(r.score for r in validation_results) / len(
            validation_results
        )
