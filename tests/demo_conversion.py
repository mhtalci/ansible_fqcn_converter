...
def simple_fqcn_conversion_demo():
    ...
    for file_path in sample_files:
        if os.path.exists(file_path):
            ...
            if non_fqcn_modules:
                print(
                    f"  ⚠️  Found {len(non_fqcn_modules)} non-FQCN modules that could be converted:"
                )
...
"""
    ...
    print(
        f"   The conversion script intelligently handles module vs parameter conflicts."
    )
    print(
        f"   Use 'python3 convert_to_fqcn.py --dry-run' to see full conversion preview."
    )
