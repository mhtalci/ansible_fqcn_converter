...
class TestJSONFormatter:
    ...
    def test_json_formatter_with_exception(self):
        ...
        record = logging.LogRecord(
            name='test.logger', level=logging.ERROR, pathname='/test/path.py',
    ...
    def test_json_formatter_with_performance_metrics(self):
        ...
        record = logging.LogRecord(
            name='test.logger', level=logging.INFO, pathname='/test/path.py',
            lineno=42, msg='Performance test', args=(), exc_info=None
        )
        record.module = 'test_module'
        record.funcName = 'test_function'
        record.process = 12345
    ...
    def test_json_formatter_with_custom_context(self):
        ...
        record = logging.LogRecord(
            name='test.logger', level=logging.INFO, pathname='/test/path.py',
            lineno=42, msg='Context test', args=(), exc_info=None
        )
        record.module = 'test_module'
        record.funcName = 'test_function'
        record.process = 12345
...
class TestSetupLogging:
    ...
    def test_setup_logging_file_permission_error(self):
        ...
        with patch('logging.config.dictConfig', side_effect=PermissionError("Permission denied")):
            with patch('logging.basicConfig') as mock_basic_config:
...
class TestLogPerformanceMetrics:
    ...
    def test_log_performance_metrics(self):
        ...
        log_performance_metrics(
            logger, 'test_operation', start_time,
            custom_metric=42, status='success'
        )
...
class TestCreateFileLogger:
    ...
    def test_create_file_logger_custom_params(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            logger = create_file_logger(
                'test.file.logger.custom', log_file,
                level='DEBUG', max_bytes=2048, backup_count=10
            )
