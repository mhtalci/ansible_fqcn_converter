...
from fqcn_converter.cli.validate import (
    add_validate_arguments,
    main,
    ValidateCommand
)
from fqcn_converter.core.validator import ValidationResult, ValidationIssue
from fqcn_converter.exceptions import ValidationError, FQCNConverterError
...
class TestValidateCommandRun:
    ...
    def test_run_unexpected_exception(self, mock_validator_class):
        ...
        with patch.object(command, '_discover_files', side_effect=RuntimeError("Unexpected")):
            result = command.run()
...
class TestValidateCommandFileDiscovery:
    ...
    def test_discover_files_directory(self):
        ...
        with patch('pathlib.Path.is_file', return_value=False):
            with patch('pathlib.Path.is_dir', return_value=True):
                with patch.object(command, '_find_ansible_files', return_value=[Path('test.yml')]):
                    files = command._discover_files()
    ...
    def test_is_ansible_file_by_extension(self):
        ...
        assert command._is_ansible_file(Path('test.yaml')) is False  # Need content check
...
class TestValidateCommandValidation:
    ...
    def test_validate_files_parallel(self, mock_executor_class, mock_validator_class):
        ...
        # Mock as_completed
        with patch('fqcn_converter.cli.validate.as_completed', return_value=[mock_future]):
...
class TestValidateCommandAnsibleLint:
    ...
    def test_run_ansible_lint_success(self, mock_run):
        ...
        mock_run.return_value.stdout = "test.yml:5:10: [E301] Commands should not change things"
...
class TestValidateCommandReporting:
    ...
    def test_generate_junit_report(self):
        ...
        mock_write.assert_called_once_with('report.xml', encoding='utf-8', xml_declaration=True)
    ...
    def test_generate_report_exception(self):
        ...
        with patch.object(command, '_generate_json_report', side_effect=Exception("Test error")):
