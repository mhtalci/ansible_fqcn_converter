...
from fqcn_converter.cli.main import main, preprocess_args
...
class TestFlexibleArgParsing:
    ...
    def test_main_with_flexible_verbose(self, mock_setup_logging, mock_convert_main):
        ...
        with patch.object(
            sys, "argv", ["fqcn-converter", "convert", "test.yml", "--verbose"]
        ):
            result = main()
    ...
    def test_main_with_flexible_quiet(self, mock_setup_logging, mock_validate_main):
        ...
        with patch.object(
            sys, "argv", ["fqcn-converter", "validate", "--quiet", "test.yml"]
        ):
            result = main()
    ...
    def test_preprocess_args_preserves_order(self):
        ...
        non_global = [
            ...
            if arg not in ["--verbose", "-v", "--quiet", "-q", "--debug", "--version"]
        ]
        expected_order = [
            "convert",
            "--config",
