...
"""
        ...
        assert any('ansible.builtin.service' in suggestion for suggestion in suggestions)
        ...
        yaml_data = [
            {
                ...
                'tasks': [
                    ...
                    {'name': 'Start service', 'ansible.builtin.service': {'name': 'nginx'}},
...
"""
        issues = [
            ValidationIssue(1, 1, 'error', 'Short module name', 'Use FQCN')
        ]
        score = validator._calculate_completeness_score(content, issues)
        assert score < 1.0
...
"""
    ...
    def test_validate_conversion_unexpected_error(self, validator, tmp_path):
        ...
        with patch.object(validator, '_validate_content', side_effect=Exception("Unexpected")):
