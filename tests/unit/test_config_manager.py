...
class TestConfigurationManager:
    ...
    def config_manager(self):
        ...
        with patch.object(
            ConfigurationManager, "_find_default_config", return_value=None
        ):
    ...
    def test_init_with_default_config(self):
        ...
        with patch.object(
            ConfigurationManager, "_find_default_config", return_value=mock_path
        ):
    ...
    def test_load_default_mappings_success(
        self, config_manager, sample_config_data, tmp_path
    ):
    ...
    def test_load_custom_mappings_success(
        self, config_manager, sample_config_data, tmp_path
    ):
    ...
    def test_load_settings_with_defaults(self, config_manager, tmp_path):
        ...
        default_config = {
            "settings": {"backup_files": False, "validation_level": "minimal"}
        }
        ...
        custom_config = {"settings": {"backup_files": True, "backup_suffix": ".custom"}}
        ...
        assert (
            settings.validation_level == "standard"
        )  # Uses built-in default since not in custom config
    ...
    def test_validate_configuration_invalid_section(self, config_manager):
        ...
        invalid_config = {"ansible_builtin": "not a dictionary"}
    ...
    def test_extract_mappings_from_config_complex_format(
        self, config_manager, sample_config_data
    ):
    ...
    def test_is_valid_fqcn_method(self, config_manager):
        ...
        assert (
            config_manager._is_valid_fqcn("community.general.docker_container") is True
        )
        ...
        assert (
            config_manager._is_valid_fqcn("ansible.builtin.") is False
        )  # Empty module
        assert (
            config_manager._is_valid_fqcn("ansible..copy") is False
        )  # Empty collection
        assert (
            config_manager._is_valid_fqcn("123.invalid.module") is False
        )  # Invalid identifier
